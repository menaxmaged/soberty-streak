...
let oldDataService = DataService.shared
...
struct Provider: AppIntentTimelineProvider {
    ...
    func timeline(for configuration: ConfigurationAppIntent, in context: Context) async -> Timeline<SimpleEntry> {
        ...
        if let selectedEvent = configuration.selectedEvent {
            event = dataService.getEvent(eventName: selectedEvent)
        }
...
struct StreakWidgetEntryView: View {
    ...
    var body: some View {
        Text(String(describing: oldDataService.listEvents()))                            .font(.system(size: 20, weight: .bold))
.foregroundStyle(.red)
